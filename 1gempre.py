...
if __name__ == "__main__":
    ...
    if template_img is not None and test_img is not None:
        ...
        plt.subplot(2, 3, 1); plt.title("Original Test"); plt.imshow(test_img, cmap='gray')
        plt.subplot(2, 3, 2); plt.title("Difference"); plt.imshow(diff_img, cmap='gray')
        plt.subplot(2, 3, 3); plt.title("Thresholded"); plt.imshow(thresh_mask, cmap='gray')
        plt.subplot(2, 3, 4); plt.title("Cleaned Mask"); plt.imshow(cleaned_mask, cmap='gray')
        # --- NEW PLOT ---
        plt.subplot(2, 3, 5); plt.title("Defects Highlighted"); plt.imshow(overlay_img)
